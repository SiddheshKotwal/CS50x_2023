...
bool check(char *word)
{
    ...
    node *tmp = root;
    for (int i = 0, str = strlen(word); i < str; i++)
    {
        int tmpnum = tolower(word[i]) - 'a';
        // check if the letter has next node
        if (tmp->children[tmpnum] != NULL)
        {
            // change the pointer to the next node
            tmp = tmp->children[tmpnum];
            // check if all the letters of the word are present and if the boolean value is true as the word ends
            if (i == (str - 1) && tmp->is_word == true)
            {
                return true;
            }
        }
